...
#   - BUILD_DIR_SRC: Output directory path
...
# Directory variables
SRC_DIR := ../Core/Src
BUILD_DIR_SRC := ./Core/Src

# Automatically find all C source files
C_SRCS := $(wildcard $(SRC_DIR)/*.c)

# Define output files
OBJS += $(patsubst $(SRC_DIR)/%.c,$(BUILD_DIR_SRC)/%.o,$(C_SRCS))
C_DEPS += $(patsubst $(SRC_DIR)/%.c,$(BUILD_DIR_SRC)/%.d,$(C_SRCS))

# Rule to build object files from C sources
$(BUILD_DIR_SRC)/%.o $(BUILD_DIR_SRC)/%.su: $(SRC_DIR)/%.c $(BUILD_DIR_SRC)/subdir.mk
	$(CC) "$<" $(CFLAGS) -o "$@"
...
clean-Core-Src:
	-$(RM) $(BUILD_DIR_SRC)/*.d $(BUILD_DIR_SRC)/*.o $(BUILD_DIR_SRC)/*.su
